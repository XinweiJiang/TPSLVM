...
%         B(1+(ci-1)*ntr:ci*ntr,1+(ci-1)*ntr:ci*ntr) - eye(ntr,ntr)./model.kern.bias;
end
...
for cm = 1:m
  sbnoiseprocxinvS4  = noiseprocxinvS4(1+(cm-1)*ntr:cm*ntr,1+(cm-1)*ntr:cm*ntr);
    ...
    bias = Theta(cm,o);	% 	bias defined by last component
    dnB = -eye(ntr,ntr)/bias/bias;
...
for ci = 1:m
    ...
    [gZ0, gHP(ci)] = tpslvmLogLikeGradients(model.modelR, gK);
