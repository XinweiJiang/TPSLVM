...
if isfield(model, 'gType') && strcmp(model.gType, 'mgpgplvm')
  fname = [outfile,'.ml'];
...
if isfield(Opt, 'classifier') && strcmp(Opt.classifier, 'Knn')
    retAcc = retAccCell.Knn;
else
    retAcc = retAccCell.Gpc;
end

filename = ['demGpgplvm' dataSetName 'Tr' num2str(size(z,1)) 'L' num2str(model.p)];
plotZ(z,model.Y,filename,isAutoClosePlot);
filename = ['demGpgplvm' dataSetName 'Te' num2str(size(zz,1)) 'L' num2str(model.p)];
plotZ(zz,model.YY,filename,isAutoClosePlot);

filename = ['demGpgplvm' dataSetName 'Tr' num2str(size(z,1)) 'Te' num2str(size(zz,1)) 'L' num2str(model.p)];
save([filename '.mat']);
