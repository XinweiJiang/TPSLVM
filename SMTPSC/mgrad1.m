function dnlHpW = mgrad1(hyper_w)
...
if isfield(model, 'trainModel') && strcmp(model.trainModel, 'combined')
    x_tr = w;
else
    x_tr = model.X*w;
end
...
for ci = 1:m
    for i = 1+ntr*(ci-1):ntr*ci
      for j = 1+ntr*(ci-1):ntr*ci
          s0 = s0 + smgpCreateFormatMatrix( i, j, ybp );
...
t= cputime;
dnlK1 = subMgrad(m, ntr, invS5, ybp, FP, diagF);
cputime-t
