function hyper_w = driverm1(data,col,outfile,DATAPATH,gx_tr,out_tr,x_te,out_te,meth,options,hyper_w,hmcopt,gmean_prior,gvar_prior,jitter2)
...
global mean_prior var_prior vec_mean_prior vec_var_prior  makepredflag
...
if (strcmp(meth,'ml') == 1) | (strcmp(meth,'ml_hmc') == 1)
  ...
  if strcmp(func2str(optim), 'minimize')
      ...
      model.isTranspose = 0;
      hyper_w = optim(hyper_w, 'smgpcLikelihoodGradients1', options(14), model);
    else
...
%   hyper_w = scg(potential,hyper_w,options,gradpot);
  
  
%   hyper_w = minimize(hyper_w', 'multiLaplaceGP', -50, model);

  
%   makepredflag = 1;
%   dummie = feval(potential,w);	% 	set global variables for makeclasspred
%   makepredflag = 0;
%   
%   [mlout,mlss] = feval(makeclasspred,w,x_tr,ntr,nte,x_all,m,jitter,B,invSxTVEC,invS4,invS5); % make predictions
%   
%   fname = [outfile,'.ml'];
%   
%   fid_w = fopen([fname,'.smp'],'w');	% store ML parameters to a file
%   mprintf(fid_w,'%f',w); fclose(fid_w);
%   
