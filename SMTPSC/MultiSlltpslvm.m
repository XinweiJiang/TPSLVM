function [ retZ, zz, retAcc ] = MultiSlltpslvm( dataSetName, x_tr, out_tr, x_te, out_te, latentDim, Opt )
...
model.modelR = tpslvmCreate(x_tr', size(x_tr,2), latentDim, options);
...
model.kern.hyper = hyper*10;
model.kern.type = 'covTps';
model.kern.length = size(hyper,1);
...
filename = ['demSlltpslvm' dataSetName 'Tr' num2str(size(model.Z,1)) 'L' num2str(model.p)];
plotZ(model.Z,model.Y,filename,isAutoClosePlot);
filename = ['demSlltpslvm' dataSetName 'Te' num2str(size(zz,1)) 'L' num2str(model.p)];
plotZ(zz,model.YY,filename,isAutoClosePlot);

retZ = model.Z;
filename = ['demSlltpslvm' dataSetName 'Tr' num2str(size(model.Z,1)) 'Te' num2str(size(zz,1)) 'L' num2str(model.p)];
save([filename '.mat']);
