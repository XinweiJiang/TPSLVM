...
m = size(out_tr,1);					% number of classes
hyper = rand(1, m*npc);			% initial paramters
...
kern.length = size(hyper,2);
model.kern = kern;
model.approx = 'cumGauss';
model.optimiser = 'scg'; % scg or minimize
...
zplusY = [x*w y];
zz = xx*w;
[resultClass, classes, distance] = kNN(zplusY, zz, 10, model);
result = resultClass - model.YY;
res = tabulate(result)

filename = [ dataSetName num2str(latentDim)];

model.Z = w'*model.X';
plotZ(model,['dem' data 'Sllgplvm']);
