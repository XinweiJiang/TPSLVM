function model = tpslvmAddDynamics(model, type, varargin)

% TPSLVMADDDYNAMICS Add a dynamics kernel to the model.
%
%	Description:
%
%	TPSLVMADDDYNAMICS(MODEL, TYPE, ...) adds a dynamics model to the
%	TPSLVM.
%	 Arguments:
%	  MODEL - the model to add dynamics to.
%	  TYPE - the type of dynamics model to add in.
%	  ... - additional arguments to be passed on creation of the
%	   dynamics model.
%	
%	
%	
%
%	See also
%	MODELCREATE


%	Copyright (c) 2005, 2006, 2007 Neil D. Lawrence


%	With modifications by Carl Henrik Ek 2008
% 	fgplvmAddDynamics.m CVS version 1.5
% 	fgplvmAddDynamics.m SVN version 82
% 	last update 2008-09-12T09:31:42.000000Z
type = [type 'Dynamics'];
model.dynamics = modelCreate(type, model.q, model.q, model.X', varargin{:});
params = tpslvmExtractParam(model);
model.dynamics.numParams = length(modelExtractParam(model.dynamics));
model.numParams = model.numParams + model.dynamics.numParams;
model = tpslvmExpandParam(model, params);

