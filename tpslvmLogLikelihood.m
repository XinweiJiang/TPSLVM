function f = tpslvmLogLikelihood(model)
...
% if model.isMAP
%     logPX = -0.5*(N*q*log(2*pi/gamma)+gamma*sum(sum(X.^2)));
% else
%     logPX = 0;
% end

f = -0.5*((D*model.logDetK)+...
    trace(model.invK*model.YtY));%+...
%     logPX; %+ LogP(X)

if isfield(model, 'dynamics') && ~isempty(model.dynamics)
    % A dynamics model is being used.
    f = f + modelLogLikelihood(model.dynamics);
...
% V2.1@2011-11-26
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% invEtETt = (1/beta*eye(N)+E'*E)\T';
% invEtETtTtY = (1/beta*eye(N)+E'*E+T'*T)\Y';
% 
% if model.isMAP == 1
